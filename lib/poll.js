...
function voteadapter(client, message, pollfile, voterslistfile) { // fique atento ao que está apos o function, isso ja ira te definir o que é
    if (isvoted(message, voterslistfile)) {
        client.reply(message.chatId, 'Que feio emm, tentando burlar a votação para votar 2 vezes? Estou de olho em você!', message.id, true);
        return;
    }
    let data = readJsonFile(pollfile)
    if (data['candis'] === 'null') {
        client.reply(message.chatId, '⭐️ Bom, não temos opcões de escolha...\nSerá que o criador da enquete esqueceu de colocar?', message.id, true);
        return;
    }
    let arr = data['candis']
...
async function addcandidate(client, message, candi, pollfile, voterslistfile) {
    ...
    if (data['candis'] === 'null') {
        ...
        if (data['candis'].length >= 10) {
            client.reply(message.chatId, '🎯️ O maximo de candidatos que posso adicionar é 10, desculpe.', message.id, true);
            return;
        }
        let cd = {
            name: candi,
    ...
    client.reply(message.chatId, `🎯️ Ok! Candidato ${candi} adicionado!`, message.id, true);
}
...
function addvote(client, message, num, pollfile, voterslistfile) {
    ...
    op = op + ls;
    op = op + `\nCaso queira votar, use *${prefix}vote <número do candidato>* , por exemplo:\n\n*${prefix}Vote 1* .`;
...
async function adminpollreset(client, message, polltitle, pollfile, voterslistfile) {
    ...
    client.reply(message.chatId, `*⭐️ *Iniciada a votação para ${polltitle}*\n\n ❤ Adicione as opções de escolha usando *${prefix}ins <Nome da Opção>* .\n\nVote com *${prefix}vote <número do candidato>*`, message.id);
    let data = {
        list: ['testentry']
    }
    saveJsonFile(voterslistfile, data);
}
var configFiles = './temp/poll/' // Localização dos arquivos de votos, não mude a menos que necessario
