const { default: got } = require('got/dist/source') // módulos e muito mais módulos
...
const wall = async (query) => { // Sobre o comando de Wallpapers
    ...
    const response = await fetch(`https://wall.alphacoders.com/api2.0/get.php?auth=${apiAlphaCoders}&method=search&count=1&term=${q}`) // Troque o API por uma API do site alpha coders
...
const ss = async (query) => { // Um print de site
    request({
        ...
        if (error) {
            ...
            fs.writeFile("./temp/img/screenshot.jpeg", body, "binary", error => {
                console.log(error);
            })
        }
    })
...
const sleep = async (ms) => { // Função parecida com a de shell usada pra esperar certo tempo antes de rodar algo, usa sistema de milissegundos
